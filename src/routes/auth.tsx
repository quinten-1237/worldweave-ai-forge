    ...
    <div className="min-h-screen flex items-center justify-center p-4 gradient-hero">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <Link to="/" className="flex items-center gap-2 justify-center mb-6">
          ...
          <span className="font-display text-xl gradient-gold-text font-bold">StoryForge AI</span>
        </Link>
        <h1 className="font-display text-2xl text-center mb-6">
          {mode === "signin" ? t("auth.title") : t("auth.signup_title")}
        </h1>

        <Button type="button" variant="outline" size="lg" className="w-full mb-4" disabled={busy} onClick={google}>
        ...
        <div className="flex items-center gap-2 my-4 text-xs text-muted-foreground"><div className="h-px flex-1 bg-border" /> {t("auth.or")} <div className="h-px flex-1 bg-border" /></div>

        <form onSubmit={submit} className="space-y-3">
          {mode === "signup" && (
            <div>
              <Label className="text-xs">{t("auth.display_name")}</Label>
              <div className="relative"><UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          ...
          <div>
            ...
            <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input className="pl-9" type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} />
            </div>
          </div>
          <Button type="submit" variant="hero" size="lg" className="w-full" disabled={busy}>
            {mode === "signin" ? t("btn.sign_in") : t("btn.sign_up")}
          </Button>
        </form>

        <button onClick={() => setMode(mode === "signin" ? "signup" : "signin")}
          className="block mx-auto mt-4 text-xs text-muted-foreground hover:text-gold">
